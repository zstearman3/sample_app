...
class UserTest < ActiveSupport::TestCase
  ...
  test "pasword should have a minimum length of 6 characters" do
  ...
  test "should follow and unfollow a user" do
    snow  = users(:snow)
    stark = users(:stark)
    assert_not snow.following?(stark)
    snow.follow(stark)
    assert snow.following?(stark)
    assert stark.followers.include?(snow)
    snow.unfollow(stark)
    assert_not snow.following?(stark)
  end
  ...
  test "feed should have the right posts" do
    ...
    stark.microposts.each do |post_following|
    ...
    tyrion.microposts.each do |post_unfollowed|
